...
function triggerScraping() {
    ...
    $command = 'python3 scrape_data.py'; // Adjust this based on your Python installation

    // Execute the command
    $output = shell_exec($command);

    // Check the output (optional - for debugging)
    if ($output) {
        echo "<p>Data scraped successfully.</p>";
    } else {
        echo "<p>Failed to scrape data.</p>";
    }
}

// Check if the form is submitted to trigger the scraping
if (isset($_POST['scrape'])) {
    triggerScraping(); // Call the function to trigger the Python script
}

// Read the scraped data from the file generated by the Python script
$scraped_data = file_get_contents('data.txt'); // Change the file name as needed
